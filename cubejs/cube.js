...
module.exports = {
  ...
  checkAuth: async (req, auth) => {
    // /meta returns schema metadata only (no actual data) — allow without token.
    // This fixes the 500 on health checks and Cube.js Playground introspection.
    const path = req.url || req.path || '';
    if (path.includes('/meta') && !auth) {
      req.securityContext = { clientId: 'nestle', role: 'analyst' };
      return;
    }
